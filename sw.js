...
self.addEventListener('install', function (event) {
  event.waitUntil(
    caches.open(CACHE_NAME).then(function (cache) {
      // B6. Add all recipe URLs to the cache on install
...
self.addEventListener('fetch', function (event) {
  ...
  /*******************************/
  // B7. Respond to the fetch event using cache-first strategy
  event.respondWith(
    caches.open(CACHE_NAME).then(function (cache) {
      return cache.match(event.request).then(function (cachedResponse) {
        // B8. Return cached version if found; otherwise fetch, cache, and return
        if (cachedResponse) {
